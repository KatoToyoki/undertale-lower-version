...
bool CGameStateRun::DetectOverlay(CMovingBitmap item)
{
	int Wbeginning = item.Left();
	int WEnd = item.Left()+item.Width();
	int Hbeginning = item.Top();
	int HEnd = item.Top()+item.Height();
	
	if(character.Left() < WEnd && character.Left()+character.Width() > Wbeginning)
	{
		if(character.Top() < HEnd && character.Top()+character.Height() > Hbeginning)
		{
			return true;
		}
	}
...
{
	if(phase == 3 && DetectOverlay(chest_and_key))
	{
		chest_and_key.SelectShowBitmap(1);
	}
	
	if(phase == 5 && DetectOverlay(door[0]))
	{
		door[0].SelectShowBitmap(1);
	}

	if(phase == 5 && DetectOverlay(door[1]))
	{
		door[1].SelectShowBitmap(1);
	}

	if(phase == 5 && DetectOverlay(door[2]))
	{
...
void CGameStateRun::OnInit()  								// 遊戲的初值及圖形設定
{
	...
	bee.CMovingBitmap::SetAnimation(2,false);
...
void CGameStateRun::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (nChar == VK_RETURN) {
		if (phase == 1) {
			...
			ball.CMovingBitmap::SetAnimation(2,true);
			ball.CMovingBitmap::ToggleAnimation();
		}else if (phase == 6) {
			if (sub_phase == 1) {
	...
	{
		character.CMovingBitmap::SetTopLeft(character.Left(),character.Top()-10);
	}
	...
	{
		character.CMovingBitmap::SetTopLeft(character.Left()+10,character.Top());
	}
